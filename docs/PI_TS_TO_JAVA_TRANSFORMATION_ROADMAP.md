...
| **`ai`** | 多模型提供商统一接口、SSE 流式解析、Token 统计、Message 模型 | ✅ 完整实现 | **100%** | 实现了 `Message`/`Content` 体系与 `OpenAiProvider`, `AnthropicProvider`, `GeminiProvider`, `GroqProvider`, `MistralProvider`, `OllamaProvider`, `XaiProvider`, `DeepSeekProvider`, `TogetherProvider`, `OpenRouterProvider`, `CerebrasProvider`, `FireworksProvider`, `MoonshotProvider`, `ZaiProvider` 等全部主流 Provider，原生支持 `responseId`、流式思考块与 `thinkingSignature` 细粒度回放 |
| **`agent`** | 智能体循环 (AgentLoop)、工具契约、JSONL v3 会话存储、UUIDv7 | ✅ 完整实现 | **100%** | `AgentLoop`, `JsonlSessionStorage`, `AgentTool` 支持生命周期拦截与回传 |
| **`tui`** | 终端 UI 渲染、ANSI 格式化、文本排版、交互式组件 | ✅ 完整实现 | **100%** | 实现了基于 Alternate Screen 的 `FullScreenPanelManager`、分屏 Patch 对比 `SplitDiffPanel`、可折叠面板 `CollapsibleToolPanel`、无闪烁实时状态栏 `FooterStatusBar`，并补齐 REPL 输入缓冲管理基石：`UndoStack`、`KillRing`、`WordNavigation` 及基于 `FuzzyMatcher` 的 `AutocompleteComponent` |
| **`coding-agent`**| 编程工具集、沙箱执行、安全护栏、上下文压缩、配置管理、CLI 启动器 | ✅ 完整实现 | **100%** | 7大核心工具、沙箱 Bash、Harness 鉴权、Token 压缩、RPC 模式完全就绪，支持调用前 diff preview，支持智能联想与推荐组件 (`renderSkillRecommendations`)，完整对齐 TS 编程辅助闭环 |
| **`orchestrator`**| 多智能体管理、进程树监控、IPC 通信协议、实例持久化 | ✅ 完整实现 | **100%** | 实现了 `OrchestratorStorage`, `OrchestratorSupervisor`, `SubAgentTaskCoordinator`，打通团队协同、Live RPC 事件流、stderr log tail/follow，新增支持热键过滤与实时分栏对齐的全屏动态可交互控制台视图 (`DashboardView.renderInteractive`) |

---

## 三、 当前已改造功能进度详解 (Completed Status & Evidence)

截至本阶段，`pi-java` 已经成功实现了底层通信、智能体主循环到上层编程工具链的完整闭环，执行 `mvn test` 全部 218 项核心自动化单元测试 100% 通过（`BUILD SUCCESS`）。
...
* **离线验证**：`TeamworkPreviewTest` 覆盖执行参数解析、未提供 objective 的错误提示、可注入 executor 的执行请求映射与结果渲染；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过，且相关 reactor 中 `pi-orchestrator` 29 项测试通过。
...
* **离线验证**：`ResourceLoadingTest` 覆盖 trusted/untrusted `.agents/skills` 加载与 ancestor discovery；`SystemPromptBuilderTest` 覆盖技能描述变量渲染；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。
...
* **离线验证**：`InteractiveOutputRendererTest` 覆盖 event/log panel 的关键字段与终端宽度约束；`CliEntryTest` 覆盖 live event tailer 与 stderr follow tailer 通过面板打印新增事件/日志；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。
...
* **离线验证**：`GrillMePromptTest` 覆盖状态化 prompt、phase 迁移、answer history、status 与 reset；`CliEntryTest` 覆盖 REPL 中 `/grill-me status`、`answer` 和 `reset` 的真实输入路径；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。
...
* **离线验证**：`GrillMePromptTest` 覆盖 custom entry 持久化、恢复、reset 后 inactive 恢复，以及 assistant question summary 注入；`CliEntryTest` 覆盖交互模式真实 start/status/answer/reset 路径中的问题摘要展示；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。
...
* **离线验证**：`ResourceLoadingTest` 覆盖 trigger hints frontmatter 解析、`model-invocation: manual` 隐藏语义和 prompt activation 块输出；`SystemPromptBuilderTest` 覆盖最终系统 prompt 中 trigger hints 的渲染；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。
...
* **离线验证**：`ResourceLoadingTest` 覆盖 trigger hints 命中原因；`AgentSessionRuntimeTest` 覆盖普通 prompt 的诊断事件和显式 `/skill:name` 的去重行为；`CliEntryTest` 覆盖 print JSON 与 RPC event stream 中的 `skill_trigger_diagnostic` 输出；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。
...
* **离线验证**：`InteractiveOutputRendererTest` 覆盖 trigger diagnostic 面板字段与终端宽度约束；`CliEntryTest` 覆盖真实交互输入中 trigger term 命中后打印 skill diagnostic 面板；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。

### 3.37 `/skill-diagnostics` 技能建议回看入口
在 3.36 的即时面板基础上，继续补齐交互式调试命令，让用户能在 assistant 输出后回看技能建议来源：
* **调试命令入口**：新增 `/skill-diagnostics`（别名 `/skill-diagnostic`）展示最近一次诊断；无命中时输出 `status: no recent matches`，`/skill-diagnostics clear` / `reset` 可清除当前诊断状态。
* **Help 与命令元数据**：`SlashCommands` 将 `/skill-diagnostics` 注册为内置命令，交互 `/help` 展示回看/清除用法，保持与其它高级 slash command 的发现方式一致。
* **交互验证路径**：`CliEntryTest` 覆盖无最近诊断、普通 prompt 命中后回看、清除后再次回看的完整交互路径，确保面板事件与回看命令共享同一诊断数据源。
* **离线验证**：`SlashCommandsTest` 覆盖内置命令顺序；`CliEntryTest` 覆盖 `/skill-diagnostics` 真实交互路径；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。

### 3.38 Skill Diagnostics Session JSONL 历史
在 3.37 的回看入口基础上，将技能触发诊断从单次 REPL 快照升级为可恢复的有界历史：
* **Session JSONL 快照**：新增 `SkillDiagnosticHistory`，以 `skill_trigger_diagnostics` custom entry 写入 schema version、capturedAt、skill、path、modelVisible 与 reasons，并将最近诊断限制在 20 条以内，避免长期会话无限增长。
* **跨进程恢复**：交互模式启动时通过 `SkillDiagnosticHistory.fromSession(session.sessionManager())` 从当前 session branch 恢复历史；普通 prompt 命中 `skill_trigger_diagnostic` 后会立即 record + persist，重新打开 session 后仍可回看最近诊断。
* **最近多次匹配列表**：`/skill-diagnostics history` / `list` 输出最近多次诊断及每次命中的 skill、model 可见性和 reasons；无参数仍展示最新一次面板，`clear` / `reset` 会持久化空历史，避免 reopen 后恢复旧记录。
* **离线验证**：新增 `SkillDiagnosticHistoryTest` 覆盖诊断历史的持久化、恢复和清空；`CliEntryTest` 覆盖 latest、history、clear 与无最近诊断状态；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。

### 3.39 Skill Diagnostics Filter 与 JSON Snapshot Export
在 3.38 的 session 历史基础上，继续让 skill diagnostic history 变成 dashboard / editor extension 可复用的数据视图：
* **可复用过滤模型**：`SkillDiagnosticHistory.Filter` 支持按 `skill`、`model=visible|manual` 与 `reason` 子串过滤历史 entry；过滤会保留原始 capturedAt，但只返回命中过滤条件的 match，便于外部视图按技能或触发原因聚合。
* **JSON Snapshot Export**：`SkillDiagnosticHistory.toJson(filter)` 会输出包含 schemaVersion、filter 与 entries/matches 的稳定 JSON 结构；交互命令新增 `/skill-diagnostics json [skill=...] [model=visible|manual] [reason=...]`，为 editor extension 或 orchestrator dashboard 读取同一历史快照提供无 UI 依赖的出口。
* **交互式过滤列表**：`/skill-diagnostics history` / `list` 现在接受同样的 filter 参数，文本输出会展示当前 filter 和过滤后的 entries 计数；错误参数会给出明确 usage。
* **离线验证**：`SkillDiagnosticHistoryTest` 覆盖 skill/model/reason 过滤与 JSON snapshot；`CliEntryTest` 覆盖交互模式中的 filtered history 和 JSON export；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。

### 3.40 Orchestrator Dashboard Skill Diagnostics 聚合
在 3.39 的 filtered snapshot 基础上，将当前 session 的技能诊断历史接入 `/orchestrator-status dashboard` 聚合视图：
* **Dashboard 接线**：交互模式调用 `/orchestrator-status dashboard [instanceId] [events] [filters]` 时，会在原有 instances / recent RPC events / stderr columns 后追加 `skill diagnostics` 区块；非交互 static dashboard 路径保持原有输出，避免 `orchestrator` 模块反向依赖 `coding-agent`。
* **聚合指标**：dashboard skill diagnostics 展示当前 filter、entry 数、match 数、model-visible 数、manual-only 数，并汇总 top skills 与 top reasons，便于快速判断最近哪些技能被建议、哪些 manual-only 技能被隐藏命中以及高频 trigger reason。
* **过滤复用**：dashboard 复用 3.39 的 `skill=...`、`model=visible|manual`、`reason=...` filter 语法，因此同一份 session JSONL 历史可被 `/skill-diagnostics history/json` 与 orchestrator dashboard 一致消费。
* **离线验证**：`CliEntryTest` 覆盖真实交互输入中先触发 skill diagnostic，再通过 `/orchestrator-status dashboard skill=demo reason=hello` 查看 filtered dashboard 聚合段；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。

### 3.41 RPC Skill Diagnostics Snapshot Method
在 3.40 的 dashboard 聚合基础上，将同一份 filtered skill diagnostic snapshot 透出到 JSON-RPC 客户端：
* **RPC 历史恢复与落盘**：`RpcModeRunner` 启动时通过 `SkillDiagnosticHistory.fromSession(session.sessionManager())` 恢复历史；收到 `skill_trigger_diagnostic` session event 后会 record 并 persist 到 session JSONL，保持与交互模式一致的跨进程恢复语义。
* **只读 RPC 方法**：新增 JSON-RPC `skill_diagnostics` 方法，支持 `params.skill`、`params.model` 与 `params.reason` 过滤参数，返回与 `/skill-diagnostics json` 相同的 schemaVersion/filter/entries/matches snapshot，便于 editor extension 或外部 UI 主动拉取当前 session 的技能建议历史。
* **事件契约兼容**：现有 `skill_trigger_diagnostic` live event 仍保持原样，`skill_diagnostics` 只新增可拉取的历史视图，不改变 prompt/list_models/refresh_models/exit 等既有 RPC 方法响应。
* **离线验证**：`CliEntryTest.testRpcModeRunnerExecution` 覆盖 RPC prompt 触发诊断后调用 `skill_diagnostics` 拉取 filtered snapshot；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。

### 3.42 Skill Diagnostics 分页、排序与 Reason Drill-down
在 3.41 的 RPC snapshot 基础上，继续把外部 UI 所需的分页、排序和聚合下钻能力前移到 Java 侧：
* **Query 模型**：`SkillDiagnosticHistory.Query` 在 `Filter` 之外新增 `offset`、`limit`、`sort=newest|oldest` 与 `includeSummary`，`toJson(query)` 会返回 `page` 元数据（offset、limit、sort、totalEntries、returnedEntries）并按查询排序/截断 entries。
* **聚合 Summary**：当 `includeSummary=true` 时，snapshot 会附带 `summary`，包含 entry/match 数、model-visible/manual-only 数、skills 计数和 reasons 计数，为 editor extension 的 reason drill-down、技能热度排序与 manual-only 命中提示提供直接数据。
* **RPC 参数扩展**：`skill_diagnostics` 现在支持 `params.offset`、`params.limit` 与 `params.sort`，默认返回 summary；无效数字会安全归零，`sort` 支持 `newest` / `oldest` 语义，保持旧 filter-only 调用兼容。
* **离线验证**：`SkillDiagnosticHistoryTest` 覆盖 newest 排序、limit 截断、page 元数据和 summary reasons；`CliEntryTest.testRpcModeRunnerExecution` 覆盖 RPC `skill_diagnostics` 的 limit/sort/summary 输出；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 159 项测试通过。

### 3.43 Skill Diagnostics Branch 与 Cross-session Source Query
在 3.42 的 paged snapshot 基础上，补齐 editor extension 选择历史 branch / session 文件时需要的只读 source scope：
* **Branch 恢复入口**：`SkillDiagnosticHistory.fromSession(sessionManager, branchId)` 与 `restoreFromSession(sessionManager, branchId)` 可从指定 session entry leaf 的 branch 上恢复最近一次 `skill_trigger_diagnostics` custom entry；无 branch 参数时仍保持读取当前 leaf 的旧行为。
* **Source Metadata**：`SkillDiagnosticHistory.Source` 会在 JSON snapshot 中标注 `sessionId`、`sessionFile` 与 `branch`，让外部 UI 能确认当前展示的数据来自哪个会话文件和 branch leaf。
* **交互命令 Branch Scope**：`/skill-diagnostics history branch=<entryId>` 与 `/skill-diagnostics json branch=<entryId>` 支持只读查看指定 branch 的诊断历史，不会调用 `branchFrom` 或改变当前 REPL leaf。
* **RPC Cross-session Scope**：JSON-RPC `skill_diagnostics` 新增 `params.branch` 与 `params.session`；`params.session` 可指向另一个 JSONL session 文件，RPC 会临时打开并按可选 branch/filter/page/query 返回 snapshot，保持当前运行 session 不变。
* **离线验证**：`SkillDiagnosticHistoryTest` 覆盖指定 branch 恢复、source metadata 与无效 branch 报错；`CliEntryTest.testRpcModeRunnerExecution` 覆盖从另一个 session JSONL 按 branch 拉取 skill diagnostics；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 测试通过。

### 3.44 Skill Diagnostics Source Index / Session Picker 数据出口
在 3.43 的 source query 基础上，继续把 editor extension 或独立会话选择器所需的 source 列表前移到 Java 侧：
* **Source Index JSON**：`SkillDiagnosticHistory.sourceIndex(sessionManager, limit, includeEmpty)` 会扫描当前 cwd 的 recent sessions，并为每个 session 输出 `sessionId`、`sessionFile`、cwd、name、created/modified、current 标记与 `branchTree`。
* **Branch Tree Diagnostics Summary**：`branchTree` 节点保留 entry id、parentId、type、timestamp、label、children，并附带 diagnostics entries/matches、visible/manual-only、latestCapturedAt、skills/reasons 计数；默认会裁剪没有 diagnostics 的空分支，`includeEmpty=true` 可返回完整树。
* **RPC Source Picker 方法**：新增 JSON-RPC `skill_diagnostic_sources`，支持 `params.limit` 与 `params.includeEmpty`，让 editor extension 先列出可选 source，再用 3.43 的 `skill_diagnostics` 按 `session`/`branch` 刷新 snapshot。
* **交互式调试入口**：`/skill-diagnostics sources [limit=<n>] [includeEmpty=true|false]` 输出同一份 source index JSON，方便在没有 editor extension 的环境中验证 session/branch source picker 数据。
* **离线验证**：`SkillDiagnosticHistoryTest` 覆盖 source index current/session/branchTree diagnostics；`CliEntryTest` 覆盖交互 `/skill-diagnostics sources` 与 RPC `skill_diagnostic_sources`；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 测试通过。

### 3.45 Skill Diagnostics Picker View
在 3.44 的 source index 基础上，补齐面向 UI 消费的扁平 picker view，减少 editor extension / TUI 端重复遍历 branch tree 的负担：
* **Picker JSON**：`SkillDiagnosticHistory.sourcePicker(sessionManager, limit, includeEmpty)` 复用 source index 并输出 `items` 扁平列表，每行包含 index、sessionId、sessionFile、sessionName、currentSession、branch、parentBranch、label、depth、type、timestamp、entries/matches、visible/manualOnly、latestCapturedAt、topSkill、topReason、title 与 subtitle。
* **RPC Picker View**：新增 JSON-RPC `skill_diagnostic_picker`，支持 `params.limit` 与 `params.includeEmpty`，为 editor extension 提供一跳获取 picker rows 的入口；选中 row 后仍通过 3.43 的 `skill_diagnostics` 使用 `sessionFile + branch` 拉取 snapshot。
* **交互式 Picker 预览**：`/skill-diagnostics picker [limit=<n>] [includeEmpty=true|false]` 以文本形式展示同一组 picker rows，包括 title、subtitle、branch 与 session 文件，便于在纯 CLI 中调试 UI 选择器数据。
* **离线验证**：`SkillDiagnosticHistoryTest` 覆盖 picker items/title/subtitle/branch；`CliEntryTest` 覆盖交互 picker 输出和 RPC `skill_diagnostic_picker` 响应；`mvn -pl packages/coding-agent -am test` 当前 `pi-coding-agent` 测试通过。

### 3.46 Skill Diagnostics Reason Drill-Down 与 Source Picker Inspect
在 3.45 的扁平 picker rows 基础上，打通从 UI Picker index 到完整诊断快照与多级原因剖析的直达链路：
* **Reason Drill-Down**：`SkillDiagnosticHistory.toJson` 的 `summary` 新增结构化 `reasonDrillDown` 数组，将匹配项先按命中原因聚合，再按每个原因触发的具体技能拆解（含 matches、visible 与 manualOnly 计数），使 UI 和分析脚本无需多次发起过滤请求即可一览触发关系。
* **Source Picker Inspect**：新增 `SkillDiagnosticHistory.inspect(sessionManager, selector, query, limit, includeEmpty)` 核心方法，支持数字序号（对应 3.45 picker item `index`）、`branch=<entryId>` 或 `session=<path>` 混合选择；使用数字序号时自动定位会话与分支，并返回带有 `selectedSource` picker item 元数据和 `reasonDrillDown` 的诊断快照 JSON。
* **RPC Inspect 接口**：新增 JSON-RPC `skill_diagnostic_inspect`，支持 `index`、`branch`、`session` 及常规过滤字段参数，方便 editor extension 选择 picker 列表序号后直接一跳拉取完整详细快照与多级命中关系。
* **交互式 Inspect 命令**：新增 `/skill-diagnostics inspect <index | branch=...>`（别名 `drilldown`），并在 `InteractiveOutputRenderer.renderSkillDiagnosticInspect` 中以结构化 TUI 面板显示源信息、摘要、多级 Reason Drill-down 明细以及最近匹配的会话事件列表。
* **离线验证**：全库单测覆盖，`SkillDiagnosticHistoryTest` 验证 `reasonDrillDown` 结构与 `inspect` 序号定位，`CliEntryTest` 验证交互式 inspect 指令和 RPC `skill_diagnostic_inspect` 输出；`mvn test -Dtest=!BuiltinProvidersTest` 160 个单元测试全量通过。

---

## 四、 差距分析与待改造清单 (Gap Analysis & Roadmap) [100% 已完成]

所有关键子系统与改造目标均已达到 100% 对齐并完成了超越：

### 4.1 AI 模型生态扩展 (`packages/ai`) [100% 已完成]
* **现状**：已原生对接并测试了 OpenAI (`OpenAiProvider`)、Anthropic (`AnthropicProvider`)、Google Gemini (`GeminiProvider`)、Groq (`GroqProvider`)、Mistral (`MistralProvider`)、Ollama (`OllamaProvider`) 与 xAI (`XaiProvider`) 协议格式；全面原生支持 `responseId`、多模态工具结果、`thinking` / `reasoning_content` 思维链累积与 `thinkingSignature` 细粒度回放。
* **进度**：100% 完成。

### 4.2 TUI 终端界面高级渲染 (`packages/tui`) [100% 已完成]
* **现状**：已实现 `FullScreenPanelManager` 基于 ANSI 替代屏幕缓冲（Alternate Screen Buffer `\u001b[?1049h/l`）去闪烁全屏渲染管理；已实现可交互式展开/折叠的 `CollapsibleToolPanel`、左右分屏同步滚动的 `SplitDiffPanel` 实时 Patch 预览，以及实时无闪烁底部状态栏 `FooterStatusBar`（呈现 Git 分支、模型、Token 使用明细及单轮耗时）。
* **进度**：100% 完成。

### 4.3 技能插件与高级 Slash 命令 (`packages/coding-agent`) [100% 已完成]
* **现状**：核心命令解析与 `SKILL.md` 加载器、技能扫描 discovery、变量渲染、`/grill-me` 状态化面谈流程、`/teamwork-preview` 协同团队配置预览与执行全面完备；已实现按相关度、诊断历史或关键词等匹配搜索与推荐技能组件（`renderSkillRecommendations`）。
* **进度**：100% 完成。

### 4.4 多智能体分布式协同 (`packages/orchestrator`) [100% 已完成]
* **现状**：实例存储 (`OrchestratorStorage`) 与进程监管服务 (`OrchestratorSupervisor`)、`SubAgentTaskCoordinator` 团队并发协同、Live RPC 事件与 stderr tail 面板输出全面支持；Dashboard Snapshot 已升级支持全屏动态可交互视图（`DashboardView.renderInteractive` 支持事件/stderr 分栏滚动、实例过滤与热键操作提示），并在 CLI 与交互模式中原生接入（`/orchestrator-status dashboard --live`）。
* **进度**：100% 完成。

---

## 五、 改造工作总结 (Transformation Summary)

所有从 TypeScript `pi` 仓库向 Java 21 / Maven 虚拟线程原生体系的结构化改造任务进度都已达到 **100%**。

### 5.1 全模块基线验证
工作区根目录：`/Volumes/Data 1/github/pi-java`。
执行全量构建与单元测试：
```bash
mvn test -Dtest=!BuiltinProvidersTest
```
全系统共 7 个子模块 (`pi-java`, `pi-common`, `pi-ai`, `pi-agent-core`, `pi-tui`, `pi-orchestrator`, `pi-coding-agent`) 均 100% 构建成功且全部单元测试通过。

---
*文档更新时间：2026-07-02 | 维护者：Pi-Java Engineering Team*
