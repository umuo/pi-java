...
Java 版插件接口已接通基础 lifecycle/tool hook、compact 事件、`user_bash` 事件、同步 `input` transform/handled 事件、同步 `tool_call` 改参/阻断，以及同步 `tool_result` 结果修改；但仍缺 TS 版动态 TS/JS 运行时，以及 provider 请求/响应、resources discover、session switch/fork 拦截、UI context、取消信号、图像输入和 streamingBehavior 等完整事件面。
...
- 可运行的扩展平台：命令、工具、事件、UI、动态 provider、resources discover。（Java 已接通 JAR SPI 的工具、命令、基础 lifecycle/tool/compact、`user_bash`、同步 `input`、`tool_call` 改参/阻断和 `tool_result` 结果修改事件；动态 TS/JS 运行时和完整 UI/provider/resources 仍待补。）
