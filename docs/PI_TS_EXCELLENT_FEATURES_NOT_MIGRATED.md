...
Java 版插件接口只有 `onBeforeTurn`、`onAfterTurn`、`onBeforeToolCall`、`onAfterToolCall` 四类简化钩子，且未接入运行时。
...
Java 有 `BashExecutor`、`ShellSupport`、`LocalBashOperations`，但交互 `InteractiveModeRunner` 没有处理 `!` / `!!` 用户命令，更多依赖模型调用 bash tool。
...
- 可运行的扩展平台：命令、工具、事件、UI、动态 provider、resources discover。
